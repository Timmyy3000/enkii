...
- The local fallback requires `actions/checkout` with `fetch-depth: 0`, Git 2.36+ (`fetch --refetch`), and a successful fetch of the exact base commit. Refetching verifies remote access even when the commit is cached, at the cost of transferring its reachable history again. A mismatched head, unavailable base, shallow history, missing/ambiguous merge base, or output over 50 MiB stops review instead of using stale refs or truncating the diff.
